...
for (let i = 0; i < userStorage.length; i++) {
  if (!userStorage[i].isActive) {
    ...
    const deviationCoefficient = document.getElementById(
    ...
    const deviationCoefficientHint = document.getElementById(
      "deviationCoefficientHint"
    );
    ...
    const calculateDeviation = (dataArray) => {
      ...
      const sumOfSquaredDifferences = numbers.reduce(
        (acc, val) => acc + Math.pow(val - mean, 2),
        0
      );
      const standardDeviation = Math.sqrt(
        sumOfSquaredDifferences / (numbers.length - 1)
      );
    ...
    window.onload = () => {
      ...
      averageValue.innerText =
        formatNumberWithSpaces(calculateAverage(turnoverByYear)) + " DH";
      averageValueHint.innerText = `Le chiffre d'affaires moyen de tous les magasins.`;

      modeValue.innerText =
        calculateMode(turnoverByYear).toLocaleString().replace(/\./g, " ") +
        " DH";
      modeValueHint.innerText = `Le chiffre d'affaires réalisé par le plus grand nombre de magasins est ${
        calculateMode(turnoverByYear).toLocaleString().replace(/\./g, " ") +
        " DH"
      }.`;

      medianValue.innerText =
        calculateMedian(turnoverByYear).toLocaleString().replace(/\./g, " ") +
        " DH";
      medianValueHint.innerText = `50% des magasins réalisent un chiffre d'affaires inférieur à ${
        calculateMedian(turnoverByYear).toLocaleString().replace(/\./g, " ") +
        " DH"
      } et, par conséquent, 50% d'entre eux réalisent un chiffre d'affaire supérieur à ce résultat.`;

      deviationCoefficient.innerText =
        calculateDeviation(turnoverByYear).toFixed(2) + "%";
      deviationCoefficientHint.innerText = `L'écart-type montre que les chiffres d'affaires des magasins s'écartent en moyenne de ${
        formatNumberWithSpaces(calculateAverage(turnoverByYear)) + " DH"
      }, et le coefficient de variation est de ${(
        (calculateDeviation(turnoverByYear) * 100) /
        calculateAverage(turnoverByYear)
      ).toFixed(4)}.`;
  ...
  <img class="arrow-icon" src="./assets/img/dropdown-icon.svg">`;
    ...
    turnoverTabButton.addEventListener("click", () => {
      if (
        workforceTabButton.classList.contains(
          "active-dashboard-navigation-button"
        )
      ) {
        workforceTabButton.classList.remove(
          "active-dashboard-navigation-button"
        );
      }

      if (
        surfaceTabButton.classList.contains(
          "active-dashboard-navigation-button"
        )
      ) {
    ...
    workforceTabButton.addEventListener("click", () => {
      if (
        turnoverTabButton.classList.contains(
          "active-dashboard-navigation-button"
        )
      ) {
        turnoverTabButton.classList.remove(
          "active-dashboard-navigation-button"
        );
      }
      ...
      averageValue.innerHTML = formatNumberWithSpaces(
        calculateAverage(workforceByYear)
      );
      averageValueHint.innerText = `L'effectif moyen de tous les magasins.`;

      modeValue.innerText = calculateMode(workforceByYear);
      modeValueHint.innerText = `L'effectif attends par le plus grand nombre de magasins est ${calculateMode(
        workforceByYear
      )}.`;

      medianValue.innerText = calculateMedian(workforceByYear);
      medianValueHint.innerText = `50% des magasins ayant un effectif inférieur à ${calculateMedian(
        workforceByYear
      )} et, par conséquent, 50% d'entre eux ayant un effectif supérieur à ce résultat.`;

      deviationCoefficient.innerText =
        calculateDeviation(workforceByYear).toFixed(2) + "%";
      deviationCoefficientHint.innerText = `L'écart-type montre que l'effectif des magasins s'écartent en moyenne de ${formatNumberWithSpaces(
        calculateAverage(workforceByYear)
      )}, et le coefficient de variation est de ${(
        (calculateDeviation(workforceByYear) * 100) /
        calculateAverage(workforceByYear)
      ).toFixed(4)}.`;
    ...
    surfaceTabButton.addEventListener("click", () => {
      ...
      averageValue.innerHTML =
        formatNumberWithSpaces(calculateAverage(surfaceByYear)) + " m²";
      averageValueHint.innerText = `Surface moyen de tous les magasins.`;

      modeValue.innerText = parseInt(calculateMode(surfaceByYear)) + " m²";
      modeValueHint.innerText = `Surface attends par le plus grand nombre de magasins est ${
        parseInt(calculateMode(surfaceByYear)) + " m²"
      }.`;

      medianValue.innerText = calculateMedian(surfaceByYear) + " m²";
      medianValueHint.innerText = `50% des magasins ayant un surface inférieur à ${
        calculateMedian(surfaceByYear) + " m²"
      } et, par conséquent, 50% d'entre eux ayant un surface supérieur à ce résultat.`;

      deviationCoefficient.innerText =
        calculateDeviation(surfaceByYear).toFixed(2) + "%";
      deviationCoefficientHint.innerText = `L'écart-type montre que surface des magasins s'écartent en moyenne de ${
        formatNumberWithSpaces(calculateAverage(surfaceByYear)) + " m²"
      }, et le coefficient de variation est de ${(
        (calculateDeviation(surfaceByYear) * 100) /
        calculateAverage(surfaceByYear)
      ).toFixed(4)}.`;
      updateWorkforcesChart(selectedOption);
    });
    ...
    dateSelect.addEventListener("click", (e) => {
      if (e.target.tagName === "DIV") {
        ...
        if (
          ...
          medianValue.innerText =
            calculateMedian(turnoverByYear)
              .toLocaleString()
              .replace(/\./g, " ") + " DH";
          medianValueHint.innerText = `50% des magasins réalisent un chiffre d'affaires inférieur à ${
            calculateMedian(turnoverByYear)
    ...
    const filteredData = (year, dataKey) => {
      return dataStorage
        .filter((data) => data.dataYear.split("/")[2] === year)
    ...
    const updateChart = (year, dataKey) => {
      ...
      fruequenceDataChart = plageDataChart.map((range) => {
        ...
        return filteredDataChart.filter(

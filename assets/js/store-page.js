...
for (let i = 0; i < userStorage.length; i++) {
  if (!userStorage[i].isActive) {
    ...
    const workforceInputModify = document.getElementById(
      "workforceInputModify"
    );
    ...
    const turnoverInputModifyHelp = document.getElementById(
      "turnoverInputModifyHelp"
    );
    const workforceInputModifyHelp = document.getElementById(
      "workforceInputModifyHelp"
    );
    const surfaceInputModifyHelp = document.getElementById(
      "surfaceInputModifyHelp"
    );

    const dashboardHeading = document.getElementById("storeDashboardHeader");
    const headingSupportingText = document.getElementById(
      "storeDashboardHeaderSupportingText"
    );
    ...
    function renderData() {
      ...
      applyEditButton.addEventListener("click", () => {
        ...
        if (yearInputModify.value === "") {
          ...
          yearInputModify.style.outline =
            "4px solid var(--error-outline-red-color)";
          isError = true;
        }

        if (
          turnoverInputModify.value === "" ||
          isNaN(turnoverInputModify.value)
        ) {
          turnoverInputModifyHelp.innerText =
            "Veuillez saisir un chiffre d'affaire valide.";
          turnoverInputModifyHelp.classList.remove("hidden");
          turnoverInputModify.style.border = "1px solid var(--red-color)";
          turnoverInputModify.style.outline =
            "4px solid var(--error-outline-red-color)";
          isError = true;
        }

        if (
          workforceInputModify.value === "" ||
          isNaN(workforceInputModify.value)
        ) {
          workforceInputModifyHelp.innerText =
            "Veuillez saisir une effectif valide.";
          workforceInputModifyHelp.classList.remove("hidden");
          workforceInputModify.style.border = "1px solid var(--red-color)";
          workforceInputModify.style.outline =
            "4px solid var(--error-outline-red-color)";
          isError = true;
        }

        if (
          surfaceInputModify.value === "" ||
          isNaN(surfaceInputModify.value)
        ) {
          surfaceInputModifyHelp.innerText =
          ...
          surfaceInputModify.style.outline =
            "4px solid var(--error-outline-red-color)";
          isError = true;
        }
    ...
    const addData = (dataYear, dataTurnover, dataWorkforce, dataSurface) => {
      const lastDataId =
        dataStorage.length > 0
          ? parseInt(dataStorage[dataStorage.length - 1].dataId)
          : 0;
